...
module Mysql_query_builder {
  ...
  class MySqlQueryBuilder: QueryBuilderBase {
    ...
    var cursor: MysqlCursor;
    var table: string;
    var _orderby_declared = false;
    var _operation_type: string;

    pragma "no doc"
    proc init(con: MysqlConnection, table: string) {
      this.conn = con;
    ...
    proc QueryAndGetCursor(): MysqlCursor {
      return this.cursor;
    }
          ...
          for op in wopcodes {
            if(i == 0) {
              if(op[2] == "IN" || op[2] == "NOT IN"){
                this.sql += " (%s %s %s) ".format(this.__quote_columns(op[1]),
                            op[2], this.__quote_values(op[3]));
              }
              else {
                this.sql += " (%s %s %s) ".format(this.__quote_columns(op[1]),
                            op[2], this.__quote_values(op[3]));
              ...
              if(op[2] == "IN" || op[2] == "NOT IN") {}
                ...
                this.sql += " %s (%s %s %s) ".format(op[4], this.__quote_columns(op[1]), 
                            op[2], this.__quote_values(op[3]));
            }
            i += 1;
          }
      ...
      if(this._has("join")) {
        ...
        try {
          for op in wopcodes {                    
            ...
            this.sql += " ON %s %s %s ".format(this.__quote_columns(op[2]), op[3],
                        this.__quote_columns(op[4]));
          }            
        }
        catch{
          writeln("Error where");
